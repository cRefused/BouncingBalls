...
var myLevel = 1;
var msgGameOver = '<span style="color: red">Game Over</span>'; 
var msgWin = '<span style="color: green">YOU WIN!<span>';
var msgNextRound = 'Press SPACE to Next Level'; 
var msgNewGame = 'Press SPACE to start New Game';

var cStageDefault, cStage;
var brickScore, stepScoreDefault, stepScore, myLives;
...
function showScore(){
	...
	document.getElementById('stage').innerHTML=myLevel;
	document.getElementById('score').innerHTML=brickScore+paddleScore;
	document.getElementById('lives').innerHTML=myLives;
	resizePadle();
	if(cStageOld < cStage){
		stepScore = stepScore + stepScoreDefault;
	}
	if(brickScore >= (maxBricks*rowBricks)*myLevel){
		gameState = false;
		gameWin = true;
		myLevel++;
