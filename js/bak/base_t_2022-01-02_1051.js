...
//	console.log('Start Game');
	...
	maxStage = 1; // максимальный уровень
...
function getBalls(){
	for(var i = 0; i < maxBalls; i++)
	{
		...
		ballSpeed = i+15 // скорость шарика
...
function ballCollision(i){ 
	...
	if(ballY[i] + dY[i] < ballRadius[i] + (brickWidth[0][0]*rowBricks)) {
		brickCollision(i);
	}
...
function brickCollision(i){
	...
	var brickIndexLX = ((brickX[brickIndexL]+1)*brickWidth[0][0]);
	var brickIndexR = ((brickIndex > maxBricks) ? maxBricks : (brickIndex + 1));
	var brickIndexRX = ((brickX[brickIndexR]-1)*brickWidth[0][0]);
	for(var r = 0; r < rowBricks; r++) {
